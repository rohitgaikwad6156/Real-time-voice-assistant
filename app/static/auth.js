(() => {
  ...
  function showAuth() {
    ...
    overlay.querySelector("#voiceAuthForm").onsubmit = async (event) => {
      ...
      try {
        const body = { email: overlay.querySelector("#authEmail").value.trim(), password: overlay.querySelector("#authPassword").value };
  ...
  async function initAuthenticatedUI() {
    try {
      ...
      sidebar.innerHTML = `
        ...
        <div class="history-label">Conversations</div><div class="history-list"></div>
      ...
      sidebar.querySelector(".history-logout").onclick = () => {
        localStorage.removeItem(TOKEN_KEY); localStorage.removeItem(USER_KEY); localStorage.removeItem(CONVERSATION_KEY); location.reload();
      };
      setTimeout(() => renderHistoryMessages(active), 400);
    } catch (err) {
      localStorage.removeItem(TOKEN_KEY); localStorage.removeItem(USER_KEY); localStorage.removeItem(CONVERSATION_KEY); showAuth();
    }
  }

  injectStyles();
  const boot = () => localStorage.getItem(TOKEN_KEY) ? initAuthenticatedUI() : showAuth();
